...
function constructΨ!(Ψ, n, D, Minv, bcleft, bcright, bctop, bcbottom, h1, h2, k1, k2)
    ...
    @views @. Ψ[3:n-1,2] = Minv[1,1]*(bctop[3:n-1]-h1) + Minv[1,2]*(bcbottom[3:n-1]-h2)
    @views @. Ψ[3:n-1,n] = Minv[2,1]*(bctop[3:n-1]-h1) + Minv[2,2]*(bcbottom[3:n-1]-h2)
...
function constructBC!(Ψ, n, D, Minv, bcleft, bcright, bctop, bcbottom, h1, h2, k1, k2)
    ...
    @inbounds @views @. Ψ[3:n-1,2] = Minv[1,1]*(bctop[3:n-1]-h1) + Minv[1,2]*(bcbottom[3:n-1]-h2)
    @inbounds @views @. Ψ[3:n-1,n] = Minv[2,1]*(bctop[3:n-1]-h1) + Minv[2,2]*(bcbottom[3:n-1]-h2)
    
    mul!(k1,Ψ2,Dcs)
    mul!(k2,Ψ2,Dce)
   
    @inbounds @views @. Ψ[2,2:n] = Minv[1,1]*(bcright[2:n]-k1) + Minv[1,2]*(bcleft[2:n]-k2)
    @inbounds @views @. Ψ[n,2:n] = Minv[2,1]*(bcright[2:n]-k1) + Minv[2,2]*(bcleft[2:n]-k2)

    return nothing
end

function f!(fu, u, p::CavityCache)
    @unpack fΨ, Ψ, n, Re, D1, D2, D4, bcleft, bcright, bctop, bcbottom, Minv, h1, h2, k1, k2, DΨ, ΨD, lDΨ, lΨD, laplΨ, biharmΨ, nonlinΨ = p
